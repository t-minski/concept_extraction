...
[/INST]
"""
prompt_zero_shot = keyword_prompt
prompt_one_shot = example_prompt + keyword_prompt
...
def extract_keywords_from_abstract(abstract):
    ...
    # Mistral7B_zero_shot
    llm_mistral = TextGeneration(generator_mistral, prompt=prompt_zero_shot)
    kw_model_mistral = KeyBERT(llm=llm_mistral, model='BAAI/bge-small-en-v1.5')
    raw_keywords = kw_model_mistral.extract_keywords([abstract], threshold=0.5)[0]
    cleaned_keywords_zero_shot = [keyword.rstrip('.') for keyword in raw_keywords if keyword.rstrip('.').lower() in abstract.lower()]
    
    # Mistral7B_one_shot
    llm_mistral = TextGeneration(generator_mistral, prompt=prompt_one_shot)
    kw_model_mistral = KeyBERT(llm=llm_mistral, model='BAAI/bge-small-en-v1.5')
    raw_keywords = kw_model_mistral.extract_keywords([abstract], threshold=0.5)[0]
    cleaned_keywords_one_shot = [keyword.rstrip('.') for keyword in raw_keywords if keyword.rstrip('.').lower() in abstract.lower()]

    return {
        "Mistral7B_KeyBERT_zero_shot": cleaned_keywords_zero_shot,
        "Mistral7B_KeyBERT_one_shot": cleaned_keywords_one_shot,
    }
...
def evaluate_keywords_from_data(base_path, datasets, extraction_functions, output_folder):
    ...
    for dataset in datasets:
        ...
        with open(os.path.join(output_folder, f'evaluation_results_mistral_{dataset}.csv'), 'w', newline='', encoding='utf-8') as csvfile:
        ...
        with open(os.path.join(output_folder, f'evaluation_results_avg_mistral_{dataset}.csv'), 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile)
...
extraction_functions = {
    "Mistral7B_KeyBERT_zero_shot": lambda abstract: extract_keywords_from_abstract(abstract)["Mistral7B_KeyBERT_zero_shot"],
    "Mistral7B_KeyBERT_one_shot": lambda abstract: extract_keywords_from_abstract(abstract)["Mistral7B_KeyBERT_one_shot"],
}
